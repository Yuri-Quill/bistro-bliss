...
import { fetchRecipes } from "../../app/slices/recipesSlice"; // Removed setPage if not needed
...
const RecipesPage = () => {
    ...
    const { loading, paginatedRecipes: { recipes, total } } = useAppSelector(
        (state) => state.recipes
    );

    const currentPage = Number(searchParams.get("page") || 1);
    const currentLimit = Number(searchParams.get("limit") || 8); // Aligned with initial state

    const totalPages = total > 0 ? Math.ceil(total / currentLimit) : 1;

    useEffect(() => {
        dispatch(fetchRecipes({ page: currentPage, limit: currentLimit, forMainPage: false }));
